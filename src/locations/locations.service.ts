...
export class LocationsService {
  ...
  async ensureCategory(categoryIdOrName: string): Promise<string> {
    ...
    category = await this.categoriesRepository.findOne({
      where: { categoryName: categoryIdOrName },
    });
  ...
  async create(
    ...
    const versions =
      locationInput.versions && locationInput.versions.length > 0
  ...
  async getUsedVersions(): Promise<string[]> {
    const result = await this.locationsRepository
      .createQueryBuilder('location')
      .select(
        'DISTINCT jsonb_array_elements_text(location.versions)',
        'version',
      )
      .getRawMany();
  ...
  async batchUpdate(
    ...
    if (locations.length === 0) {
      throw new NotFoundException('No locations found with the provided IDs');
    }
    ...
    for (const location of locations) {
      ...
      if (updateData.category) {
        categoryId = await this.ensureCategory(updateData.category);
      }

      // Prepare update data (excluding category since we use categoryId)
      const { category, ...cleanUpdateData } = updateData;
      ...
      // Update the location
      await this.locationsRepository.update(location.id, finalUpdateData);
    ...
    // Log batch update to changelog
    await this.changelogService.logLocationBatchUpdate(ids, updateData, userId);
